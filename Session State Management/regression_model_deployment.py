...
def makePrediction(pipe, miles, year, make, model, engineSize, province):
    features = [miles, year, make, model, engineSize, province]
...
if __name__ == "__main__":
    ...
    with st.form(key="form"):
       ...
       'Prius c', 'Prius C', 'Prius v'])
        ...
        st.form_submit_button("Calculate", type="primary", on_click=makePrediction, kwargs=dict(
                pipe = pipe,
                miles = st.session_state["miles"], 
                year = st.session_state["year"], 
                make = st.session_state["make"], 
                model = st.session_state["model"], 
                engineSize = st.session_state["engine_size"], 
